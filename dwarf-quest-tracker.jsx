...
const SHARED_KEY = "dwarfquest-shared-v5";
...
export default function App() {
  ...
  const editingRef = useRef(false); // pause poll overwrite while a text field is focused
  const sharedRef = useRef(SHARED_DEFAULTS); // last known-good state — never let a transient error wipe this
  const patchQueueRef = useRef(Promise.resolve()); // serializes saves so rapid edits can't race and clobber each other
  ...
  const fetchShared = useCallback(async (applyToState = true, allowDefaultFallback = false) => {
    try {
      const res = await window.storage.get(SHARED_KEY, true);
      const data = res && res.value ? JSON.parse(res.value) : SHARED_DEFAULTS;
      if (applyToState && !editingRef.current) setShared(data);
      return data;
    } catch (e) {
      // Only a brand-new, never-saved campaign should fall back to blank defaults.
      // Any later read failure is almost certainly transient — keep showing what's already on screen.
      if (allowDefaultFallback) {
        if (applyToState) setShared(SHARED_DEFAULTS);
  ...
  // poll
  useEffect(() => {
    const t = setInterval(() => fetchShared(), POLL_MS);
    return () => clearInterval(t);
  }, [fetchShared]);

  // fetch-modify-write, serialized so overlapping edits (e.g. rapid slider drags) can never
  // read stale data and clobber each other's writes — each save waits for the previous to finish.
  const patchShared = useCallback((mutator) => {
    const run = async () => {
      // A read can fail for two very different reasons: the shared key genuinely doesn't
      // exist yet (normal for a brand-new campaign, before its first-ever save), or something
      // actually went wrong (a real transient error). Only the second case should be retried
      // and, if it persists, aborted rather than risk writing from a stale local copy that
      // might be missing another device's more recent changes. We tell them apart using
      // whether this device has ever successfully loaded real data this session.
      const neverLoadedYet = sharedRef.current === SHARED_DEFAULTS;
      let base = null;
      let readOk = false;
      ...
      if (neverLoadedYet) {
        try {
          const res = await window.storage.get(SHARED_KEY, true);
          base = res && res.value ? JSON.parse(res.value) : SHARED_DEFAULTS;
        } catch (e) {
          base = SHARED_DEFAULTS; // likely just means the campaign doesn't exist yet — proceed
        }
        readOk = true;
      } else {
        for (let attempt = 0; attempt < 3 && !readOk; attempt++) {
          try {
            const res = await window.storage.get(SHARED_KEY, true);
            base = res && res.value ? JSON.parse(res.value) : sharedRef.current;
            readOk = true;
          } catch (e) {
      ...
      if (!readOk) {
        showToast("Couldn't reach the shared campaign — that change wasn't saved, please try again");
        return;
      }
      let updated;
      try {
        updated = mutator(base) || base;
      } catch (e) {
        showToast("That change couldn't be applied — please try again");
        return;
      }
      setShared(updated);
      try {
        await window.storage.set(SHARED_KEY, JSON.stringify(updated), true);
      } catch (e) {
  ...
  const chooseIdentity = async (next) => {
    setIdentity(next);
    try { await window.storage.set(IDENTITY_KEY, JSON.stringify(next), false); } catch (e) {}
  };
  ...
  if (!identity) {
    return <IdentityGate shared={shared} onChoose={chooseIdentity} onRefresh={() => fetchShared()} />;
  }

  if (identity.role === "command") {
    return <CommandConsole shared={shared} patchShared={patchShared} onSwitch={switchIdentity} toast={toast} showToast={showToast} editingRef={editingRef} onManualRefresh={() => fetchShared()} gmName={identity.gmName} />;
  }
  ...
  return <PlayerConsole shared={shared} patchShared={patchShared} me={me} onSwitch={switchIdentity} toast={toast} showToast={showToast} editingRef={editingRef} onManualRefresh={() => fetchShared()} />;
}
